...
import { Box, FormControl, IconButton, NativeSelect, Paper, Stack, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Typography, useMediaQuery, useTheme } from "@mui/material";
...
const Overview = ({ data }) => {
  ...
  const cloneEl = data.cardData.map((card, index) =>
    React.cloneElement(<Card dataCard={card} />, { key: index })
  );
  ...
  return (
    <Box sx={{display: 'flex', flexDirection: 'row', alignItems: 'start', gap: 2, ml: -10}}>
      ...
      {useMediaQuery("(min-width:1201px)") && <UserData data={data} /> }
    </Box>
  );
};
